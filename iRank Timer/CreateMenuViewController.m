...
-(void)createManualTimer:(id)sender {
    ...
    if( IS_IPAD ){
        ...
        BlindLevel *blindLevel = [[BlindLevel alloc] initWithLevelNumber:1 levelIndex:timerEditViewController.blindSet.blindLevels smallBlind:0 bigBlind:0 ante:0 duration:0];
        ...
        [self performSegueWithIdentifier:@"CreateManualBlindSetSegue" sender:self];
    }
}
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if( [segue.identifier isEqualToString:@"CreateManualBlindSetSegue"] ){
        
        TimerEditMasterViewController *viewController = (TimerEditMasterViewController *)segue.destinationViewController;
        
        viewController.blindSet       = [[BlindSet alloc] init];
        viewController.blindSet.isNew = YES;
        
        BlindLevel *blindLevel = [[BlindLevel alloc] initWithLevelNumber:1 levelIndex:viewController.blindSet.blindLevels smallBlind:0 bigBlind:0 ante:0 duration:0];
        [viewController.blindSet addBlindLevel:blindLevel];
        
        viewController.blindSet.blindSetName = NSLocalizedString(@"New blind set", nil);
        [viewController.blindSet resetLevelNumbers];
    }
}
